...
import 'package:first_page/categories_pages.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _fetchHomeData() async {
    try {
      List<InvestmentItem> interests =
          await _fetchProjectSection('${_baseUrl}interest-projects/');
      List<InvestmentItem> topRaised =
          await _fetchProjectSection('${_baseUrl}top-raised/');
      List<InvestmentItem> closingSoon =
          await _fetchProjectSection('${_baseUrl}closing-soon/');
      List<InvestmentItem> trending =
          await _fetchProjectSection('${_baseUrl}trending/');
  ...
  Future<void> _fetchSavedItems() async {
    try {
      final response =
          await http.get(Uri.parse('${_baseUrl}user/saved_projects/'));
      if (response.statusCode == 200) {
        final List<dynamic> data = jsonDecode(response.body);
        setState(() {
          _savedItems = data
  ...
  Future<void> _toggleBookmark(InvestmentItem item) async {
    final bool isCurrentlySaved = _savedItems.any((savedItem) =>
        savedItem.title == item.title); // Prefer using item.id if available
                    ...
                    if (snapshot.connectionState == ConnectionState.waiting &&
                      ...
                      return Column(
                        children: [
                          ...
                          _buildInvestmentSection(allProjects
                              .where(
        ...
        onChanged: (query) {
          setState(() {
            if (query.isNotEmpty) {
              ...
              _searchProjects(
                  query); // Call API for search (if not already handled by FutureBuilder)
            } else {
  ...
  Widget _buildInvestmentList(List<InvestmentItem> list) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isSaved = _savedItems
            .any((s) => s.title == item.title); // Use item.id if available
        return Padding(
